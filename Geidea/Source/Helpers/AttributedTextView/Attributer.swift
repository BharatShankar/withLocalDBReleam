...
//    fileprivate var urlCallbacks: [String : ((_ link: String) -> ())] = [:]
    ...
    // MARK: Customised
    /**
     Called when tapped on link in textview
     */
    public var urlCallbackByGuesture: ((_ link: String) -> ())? = nil
    ...
    // MARK: Customised
    // This will make string clickable by setting attribute of link
    open func makeClickableAsLink() -> Attributer {
        for nsRange in self.ranges {
            let iRange = self.attributedText.string.range(from: nsRange)
            if let escapedString = String(self.attributedText.string[iRange!]).addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlHostAllowed) {
                self.attributedText.addAttribute(NSAttributedString.Key.link, value: escapedString, range: nsRange)
            }
        }
        return self
    }
    
    // Helps to set handler, which will be called when link tapped in textview
    open func clickableTextHandler(_ callback : @escaping ((_ link: String) -> ())) -> Attributer {
        self.urlCallbackByGuesture = callback
        return self
    }
    
    /// Helps to find and set strings, so that can be clickable
    open func findAndMakeStringsClickableAsLink(stringsToFind : [String]) -> Attributer {

        ranges.removeAll()
        
        stringsToFind.forEach { (string) in
            let iRange = self.attributedText.string.range(of: string)
            let nsRange = (self.attributedText.string as NSString).range(of: string)
            if let escapedString = String(self.attributedText.string[iRange!]).addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlHostAllowed) {
                self.attributedText.addAttribute(NSAttributedString.Key.link, value: escapedString, range: nsRange)
            }
